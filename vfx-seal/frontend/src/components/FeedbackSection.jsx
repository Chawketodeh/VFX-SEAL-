...
export default function FeedbackSection({ vendorId, vendorName }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setSuccess('Feedback submitted! It will be visible after admin approval.');
            setRating(0);
            setMessage('');
            fetchFeedbacks();
        } catch (err) {
    ...
    const renderStars = (value, size = 'md') => {
        return (
            <div className={`stars stars-${size}`}>
                {[1, 2, 3, 4, 5].map(star => (
                    <span key={star} className={`star ${star <= value ? 'filled' : ''}`}>★</span>
                ))}
            </div>
        );
    };
                    ...
                    <div className="accordion-content">
                        ...
                        <div className="feedback-summary">
                            <div className="feedback-summary-left">
                                <div className="feedback-avg-rating">{avgRating.toFixed(1)}</div>
                                {renderStars(Math.round(avgRating), 'lg')}
                                <div className="feedback-total">{totalRatings} review{totalRatings !== 1 ? 's' : ''}</div>
                            </div>
                        </div>
                                        ...
                                        {fb.status === 'REJECTED' ? (
                                            <>
                                                ...
                                                <div className="feedback-rejected-content">
                                                    <div className="feedback-item-header">
                                                        <div className="feedback-item-studio">
                                                            ...
                                                            <div>
                                                                <div className="feedback-item-name">{fb.studioName}</div>
                                                                <div className="feedback-item-date">{formatDate(fb.createdAt)}</div>
                                                            </div>
                                                        </div>
                                                ...
                                                <div className="feedback-rejected-notice">
                                                    This review was removed due to inappropriate or violent content.
                                                </div>
                                            </>
                                        ) : (
                                            <>
                        ...
                        {/* My Feedback Status */}
                        {myFeedback && (
                            <div className={`feedback-my-status ${myFeedback.status.toLowerCase()}`}>
                                <div className="feedback-my-status-header">
                                    <span>Your Feedback</span>
                                    <span className={`status-badge ${myFeedback.status.toLowerCase()}`}>{myFeedback.status}</span>
                                </div>
                        ...
                        {!isAdmin && !myFeedback && (
                            <div className="feedback-form-card">
                                ...
                                <p className="feedback-form-subtitle">Share your experience with {vendorName}</p>
                                ...
                                <form onSubmit={handleSubmit}>
                                    ...
                                    <div className="form-group">
                                        <label className="form-label" htmlFor="feedback-message">Your Review</label>
                                        <textarea
